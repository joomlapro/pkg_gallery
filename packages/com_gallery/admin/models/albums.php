...
class GalleryModelAlbums extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__gallery_albums') . ' AS a');
		...
		if (JLanguageAssociations::isEnabled())
		{
			$query->select('COUNT(asso2.id)>1 as association')
				->join('LEFT', '#__associations AS asso ON asso.id = a.id AND asso.context=' . $db->quote('com_gallery.item'))
		...
		if (is_numeric($tagId))
		{
			$query->where($db->quoteName('tagmap.tag_id') . ' = ' . (int) $tagId)
				->join(
					...
					. ' AND ' . $db->quoteName('tagmap.type_alias') . ' = ' . $db->quote('com_gallery.album')
				);
		}
	...
	public function getAuthors()
	{
		...
		$query->select('u.id AS value, u.name AS text')
			...
			->join('INNER', $db->quoteName('#__gallery_albums') . ' AS c ON c.created_by = u.id')
	...
	public function getItems()
	{
		...
		if (JFactory::getApplication()->isSite())
		{
			...
			{
				// Check the access level. Remove albums the user should not see.
