...
/**
 * Methods supporting a list of picture records.
 *
 * @package     Gallery
 * @subpackage  com_gallery
 * @author      Bruno Batista <bruno.batista@ctis.com.br>
 * @since       3.2
 */
class GalleryModelPictures extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->select(
			$this->getState(
				'list.select',
				'a.id, a.album_id, a.title, a.checked_out, a.checked_out_time' .
					', a.state, a.ordering, a.created, a.created_by, a.created_by_alias'
			)
		);
		$query->from($db->quoteName('#__gallery_albums_pictures') . ' AS a');
	...
	public function getAuthors()
	{
		...
		$query->select('u.id AS value, u.name AS text')
			...
			->join('INNER', $db->quoteName('#__gallery_albums_pictures') . ' AS c ON c.created_by = u.id')
