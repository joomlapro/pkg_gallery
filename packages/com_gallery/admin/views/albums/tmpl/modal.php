...
?>
<form action="<?php echo JRoute::_('index.php?option=com_gallery&view=albums&layout=modal&tmpl=component&function=' . $function . '&' . JSession::getFormToken() . '=1'); ?>" method="post" name="adminForm" id="adminForm" class="form-inline">
	<fieldset class="filter clearfix">
		<div class="btn-toolbar">
			...
			<div class="btn-group pull-left">
				<input type="text" name="filter_search" id="filter_search" value="<?php echo $this->escape($this->state->get('filter.search')); ?>" size="30" title="<?php echo JText::_('COM_GALLERY_FILTER_SEARCH_DESC'); ?>" />
			</div>
			<div class="btn-group pull-left">
		...
		<div class="filters pull-left">
			...
			<?php if ($this->state->get('filter.forcedLanguage')): ?>
				<select name="filter_category_id" id="filter_category_id" class="input-medium" onchange="this.form.submit()">
					...
					<?php echo JHtml::_('select.options', JHtml::_('category.options', 'com_gallery', array('filter.language' => array('*', $this->state->get('filter.forcedLanguage')))), 'value', 'text', $this->state->get('filter.category_id')); ?>
				</select>
			...
			<?php else: ?>
				<select name="filter_category_id" id="filter_category_id" class="input-medium" onchange="this.form.submit()">
					<option value=""><?php echo JText::_('JOPTION_SELECT_CATEGORY'); ?></option>
					<?php echo JHtml::_('select.options', JHtml::_('category.options', 'com_gallery'), 'value', 'text', $this->state->get('filter.category_id')); ?>
				</select>
		...
		<tbody>
			<?php foreach ($this->items as $i => $item):
				...
				<tr class="row<?php echo $i % 2; ?>">
					<td>
						<a href="javascript:void(0)" onclick="if (window.parent) window.parent.<?php echo $this->escape($function); ?>('<?php echo $item->id; ?>', '<?php echo $this->escape(addslashes($item->title)); ?>', '<?php echo $this->escape($item->catid); ?>', null, '<?php echo $this->escape(GalleryHelperRoute::getAlbumRoute($item->id, $item->catid, $item->language)); ?>', '<?php echo $this->escape($lang); ?>', null);"><?php echo $this->escape($item->title); ?></a>
					</td>
					<td class="center">
