 ...
 * @since       3.2
 */
class GalleryModelAlbums extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':' . $this->getState('filter.album_id.include');
	...
	/**
	 * Get the master query for retrieving a list of albums subject to the model state.
	 *
	...
	protected function getListQuery()
	{
		...
		// Process an Archived Album layout.
		if ($this->getState('filter.state') == 2)
		{
			// If badcats is not null, this means that the album is inside an archived category.
			// In this case, the state is set to 2 to indicate Archived (even if the album state is Published).
		...
		// Join over the frontpage albums.
		if ($this->context != 'com_gallery.featured')
		{
			$query->join('LEFT', '#__gallery_albums_frontpage AS fp ON fp.album_id = a.id');
		}
		...
		{
			// Use album state if badcats.id is null, otherwise, force 0 for unpublished.
		...
		// Filter by a single or group of albums.
		$albumId = $this->getState('filter.album_id');

		if (is_numeric($albumId))
		{
			$type = $this->getState('filter.album_id.include', true) ? '= ' : '<> ';
			$query->where('a.id ' . $type . (int) $albumId);
		}
		elseif (is_array($albumId))
		{
			...
			$type = $this->getState('filter.album_id.include', true) ? 'IN' : 'NOT IN';
			$query->where('a.id ' . $type . ' (' . $albumId . ')');
		}
	...
	public function getItems()
	{
		...
		$globalParams = JComponentHelper::getParams('com_gallery', true);

		// Convert the parameter fields into objects.
		foreach ($items as &$item)
		{
			$albumParams = new JRegistry;
			$albumParams->loadString($item->params);

			// Unpack readmore and layout params.
			$item->alternative_readmore = $albumParams->get('alternative_readmore');
			$item->layout = $albumParams->get('layout');
			$item->params = clone $this->getState('params');

			/*
			For blogs, album params override menu item params only if menu param = 'use_album'.
			Otherwise, menu item params control the layout.
			If menu item is 'use_album' and there is no album param, use global.
			*/
			if ($input->getString('option') == 'com_gallery' && (($input->getString('layout') == 'blog') || ($input->getString('view') == 'featured') || ($this->getState('params')->get('layout_type') == 'blog')))
			{
				// Create an array of just the params set to 'use_album'.
				...
				foreach ($menuParamsArray as $key => $value)
				{
					if ($value === 'use_album')
					{
						// If the album has a value, use it.
						if ($albumParams->get($key) != '')
						{
							// Get the value from the album.
							$albumArray[$key] = $albumParams->get($key);
						}
						else
						{
							// Otherwise, use the global value.
							$albumArray[$key] = $globalParams->get($key);
						}
					}
				}

				// Merge the selected album params.
				if (count($albumArray) > 0)
				{
					$albumParams = new JRegistry;
					$albumParams->loadArray($albumArray);
					$item->params->merge($albumParams);
				}
			}
			else
			{
				// For non-blog layouts, merge all of the album params.
				$item->params->merge($albumParams);
			}
			...
			if (!$guest)
			{
				$asset = 'com_gallery.album.' . $item->id;
			...
			if ($access)
			{
				// If the access filter has been set, we already have only the albums this user can view.
			...
			$item->tags->getItemTags('com_gallery.album', $item->id);
		}
